...
import type {
  ExternalPaymentTableSettings,
  ExternalPaymentVisibleField,
} from "@/lib/payment";

type ExternalPaymentSettingsModalProps = {
  isOpen: boolean;
  settings: ExternalPaymentTableSettings;
  onClose: () => void;
  onSave: (settings: ExternalPaymentTableSettings) => void;
};

const fieldOptions: Array<{
  description: string;
  label: string;
  value: ExternalPaymentVisibleField;
}> = [
  { label: "Identificador", value: "id", description: "Codigo interno do pagamento externo" },
  ...
  { label: "Valor em credito", value: "valorCredito", description: "Valor movimentado em credito" },
  {
    ...
    value: "valorDinheiro",
    description: "Equivalente financeiro registrado",
  },
];

export function ExternalPaymentSettingsModal({
  isOpen,
  settings,
  onClose,
  onSave,
}: ExternalPaymentSettingsModalProps) {
  const [shouldRender, setShouldRender] = useState(isOpen);
  const [isVisible, setIsVisible] = useState(false);
  const [draft, setDraft] = useState<ExternalPaymentTableSettings>(settings);
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm leading-7 text-[var(--muted)]">
              Defina quantos pagamentos externos aparecem por pagina e quais colunas ficam visiveis.
            </p>
          </div>
