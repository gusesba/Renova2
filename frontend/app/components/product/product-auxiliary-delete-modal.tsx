...
type ProductAuxiliaryDeleteModalProps = {
...
export function ProductAuxiliaryDeleteModal({
  auxiliaryLabel,
  auxiliaryName,
  isOpen,
  isSubmitting,
  onClose,
  onConfirm,
}: ProductAuxiliaryDeleteModalProps) {
  ...
  return (
    <div
      className={`fixed inset-0 z-[60] flex items-center justify-center bg-[rgba(15,23,42,0.52)] p-4 transition-opacity duration-200 ease-out ${
    ...
    >
      ...
      >
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.16em] text-rose-500">
                Excluir {auxiliaryLabel.toLowerCase()}
              </p>
              ...
              <p className="mt-2 text-sm leading-7 text-[var(--muted)]">
                {auxiliaryName
                  ? `Voce esta prestes a excluir ${auxiliaryLabel.toLowerCase()} ${auxiliaryName}. A exclusao so sera permitida quando nao houver nenhum produto vinculado.`
                  : `Voce esta prestes a excluir este ${auxiliaryLabel.toLowerCase()}. A exclusao so sera permitida quando nao houver nenhum produto vinculado.`}
              </p>
            </div>
          </div>
        ...
        <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:justify-end">
          ...
          >
            {isSubmitting ? `Excluindo ${auxiliaryLabel.toLowerCase()}...` : "Confirmar exclusao"}
          </button>
        </div>
      </div>
    </div>
