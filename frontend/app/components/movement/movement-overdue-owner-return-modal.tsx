...
type MovementOverdueOwnerReturnModalProps = {
  clientName: string;
  isOpen: boolean;
  productCount: number;
  onClose: () => void;
...
export function MovementOverdueOwnerReturnModal({
  clientName,
  isOpen,
  productCount,
  onClose,
  onConfirm,
}: MovementOverdueOwnerReturnModalProps) {
    ...
    >
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-semibold tracking-tight text-[var(--foreground)]">
              Criar devolucao ao dono?
            </h2>
            <p className="mt-2 text-sm leading-7 text-[var(--muted)]">
              Alguma(s) peca(s) do cliente {clientName} ja passaram do tempo de permanencia e
              continuam em estoque. Deseja abrir uma nova movimentacao de devolucao ao dono com{" "}
              {productCount} peca(s)?
            </p>
          </div>
        ...
        <div className="mt-6 rounded-2xl border border-[var(--border)] bg-[var(--surface-muted)] px-4 py-4 text-sm text-[var(--muted)]">
          Se confirmar, a tela vai abrir uma nova aba de movimentacao do tipo devolucao dono com
          essas pecas ja vinculadas.
        </div>
