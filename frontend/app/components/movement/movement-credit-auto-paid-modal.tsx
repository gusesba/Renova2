...
type MovementCreditAutoPaidModalProps = {
  credit: CreditAutoPaidPrompt | null;
  isOpen: boolean;
  onClose: () => void;
};

export function MovementCreditAutoPaidModal({
  credit,
  isOpen,
  onClose,
}: MovementCreditAutoPaidModalProps) {
  ...
  if (typeof document === "undefined" || !shouldRender || !credit) {
    return null;
  }
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-semibold tracking-tight text-[var(--foreground)]">
              Conta paga com creditos da loja
            </h2>
            <p className="mt-2 text-sm leading-7 text-[var(--muted)]">
              {credit.nome ? `${credit.nome} ` : "O cliente "}ja tinha saldo suficiente, entao a
              conta foi paga automaticamente com os creditos disponiveis.
            </p>
          </div>
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2">
          <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface-muted)] px-4 py-4">
            ...
            <p className="mt-2 text-2xl font-semibold text-[var(--foreground)]">
              {formatCurrency(credit.creditoAnterior)}
            </p>
          </div>
          ...
          <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface-muted)] px-4 py-4">
            ...
            <p className="mt-2 text-2xl font-semibold text-[var(--foreground)]">
              {formatCurrency(credit.creditoAtual)}
            </p>
          </div>
        </div>
