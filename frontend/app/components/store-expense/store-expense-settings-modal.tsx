...
import type {
  StoreExpenseTableSettings,
  StoreExpenseVisibleField,
} from "@/lib/store-expense";

type StoreExpenseSettingsModalProps = {
  isOpen: boolean;
  settings: StoreExpenseTableSettings;
  onClose: () => void;
  onSave: (settings: StoreExpenseTableSettings) => void;
};

const fieldOptions: Array<{
  description: string;
  label: string;
  value: StoreExpenseVisibleField;
}> = [
  { label: "Data", value: "data", description: "Data de lancamento do gasto" },
  { label: "Natureza", value: "natureza", description: "Tipo de entrada ou saida" },
  { label: "Valor", value: "valor", description: "Valor financeiro do lancamento" },
  { label: "Descricao", value: "descricao", description: "Observacao do gasto" },
  { label: "Identificador", value: "id", description: "Codigo interno do lancamento" },
];

export function StoreExpenseSettingsModal({
  isOpen,
  settings,
  onClose,
  onSave,
}: StoreExpenseSettingsModalProps) {
  const [shouldRender, setShouldRender] = useState(isOpen);
  const [isVisible, setIsVisible] = useState(false);
  const [draft, setDraft] = useState<StoreExpenseTableSettings>(settings);
