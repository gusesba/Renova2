...
import type { SolicitacaoTableSettings, SolicitacaoVisibleField } from "@/lib/solicitacao";

type SolicitacaoSettingsModalProps = {
  isOpen: boolean;
  settings: SolicitacaoTableSettings;
  onClose: () => void;
  onSave: (settings: SolicitacaoTableSettings) => void;
};

const fieldOptions: Array<{
  description: string;
  label: string;
  value: SolicitacaoVisibleField;
}> = [
  { label: "Produto", value: "produto", description: "Referencia principal desejada" },
  { label: "Descricao", value: "descricao", description: "Detalhe livre da solicitacao" },
  { label: "Marca", value: "marca", description: "Marca desejada" },
...
export function SolicitacaoSettingsModal({
  ...
  onClose,
  onSave,
}: SolicitacaoSettingsModalProps) {
  const [shouldRender, setShouldRender] = useState(isOpen);
  const [isVisible, setIsVisible] = useState(false);
  const [draft, setDraft] = useState<SolicitacaoTableSettings>(settings);
  ...
  return (
    ...
    >
      <div
        className={`max-h-[calc(100dvh-env(safe-area-inset-top,0px)-env(safe-area-inset-bottom,0px)-2.5rem)] w-full max-w-3xl overflow-y-auto rounded-[28px] border border-[var(--border)] bg-white p-6 shadow-[0_30px_90px_rgba(15,23,42,0.22)] transition duration-250 ease-out sm:max-h-[calc(100vh-2rem)] ${
