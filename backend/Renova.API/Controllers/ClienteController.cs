...
namespace Renova.API.Controllers
{
    ...
    public class ClienteController(IClienteService clienteService, RenovaDbContext context) : AuthenticatedControllerBase(context)
    {
        ...
        [HttpGet("minhas-pecas")]
        [ProducesResponseType(typeof(PaginacaoDto<ClienteProdutoAreaDto>), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        public async Task<IActionResult> GetMinhasPecas([FromQuery] ObterMinhasPecasQuery query, CancellationToken cancellationToken)
        {
            int? usuarioId = await ObterUsuarioIdAsync(cancellationToken);
            ...
            try
            {
                PaginacaoDto<ClienteProdutoAreaDto> resultado = await _clienteService.GetMyProductsAsync(
                    query,
                    usuarioId.Value,
                    cancellationToken);
        ...
        [ProducesResponseType(typeof(PaginacaoDto<ClienteProdutoAreaDto>), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        public async Task<IActionResult> GetMeusProdutos([FromQuery] ObterMinhasPecasQuery query, CancellationToken cancellationToken)
        {
            int? usuarioId = await ObterUsuarioIdAsync(cancellationToken);
            ...
            try
            {
                PaginacaoDto<ClienteProdutoAreaDto> resultado = await _clienteService.GetMyCustomerProductsAsync(
                    query,
                    usuarioId.Value,
                    cancellationToken);
        ...
        [HttpGet("fechamento/produtos/exportar")]
        [ProducesResponseType(typeof(FileContentResult), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        public async Task<IActionResult> ExportarFechamentoProdutosClientes(
            [FromQuery] ExportarFechamentoClientesQuery query,
            CancellationToken cancellationToken)
        {
            int? usuarioId = await ObterUsuarioIdAsync(cancellationToken);
            ...
            try
            {
                byte[] arquivo = await _clienteService.ExportProductClosingAsync(
                    query,
                    new ObterClientesParametros
                    {
                        UsuarioId = usuarioId.Value
                ...
                return File(
                    arquivo,
                    "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    $"fechamento-clientes-produtos-{dataInicial}-{dataFinal}.xlsx");
            }
            catch (ArgumentException ex)
            {
                return BadRequest(new { mensagem = ex.Message });
        ...
        [HttpGet("fechamento/movimentacoes/exportar")]
        [ProducesResponseType(typeof(FileContentResult), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        public async Task<IActionResult> ExportarFechamentoMovimentacoesClientes(
            [FromQuery] ExportarFechamentoClientesQuery query,
            CancellationToken cancellationToken)
        {
            int? usuarioId = await ObterUsuarioIdAsync(cancellationToken);
            ...
            try
            {
                byte[] arquivo = await _clienteService.ExportMovementClosingAsync(
                    query,
                    new ObterClientesParametros
                    {
                        UsuarioId = usuarioId.Value
                ...
                return File(
                    arquivo,
                    "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    $"fechamento-clientes-movimentacoes-{dataInicial}-{dataFinal}.xlsx");
            }
            catch (ArgumentException ex)
            {
                return BadRequest(new { mensagem = ex.Message });
