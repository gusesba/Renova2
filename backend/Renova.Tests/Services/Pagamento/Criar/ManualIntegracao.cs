...
namespace Renova.Tests.Services.Pagamento.Criar
{
    ...
    {
        [Fact]
        public async Task PostPagamentoManualDeveRetornarCreatedQuandoComandoForValido()
        {
            await using RenovaApiFactory factory = new();
            HttpClient client = factory.CreateClient();

            UsuarioTokenDto autenticacao = await CriarUsuarioAutenticadoAsync(client, "manual-api@renova.com");
            ...
            HttpResponseMessage response = await client.PostAsJsonAsync("/api/pagamento/manual", new CriarPagamentoManualCommand
            {
                LojaId = loja.Id,
                ...
                Descricao = "Lancamento administrativo"
            });

            Assert.Equal(HttpStatusCode.Created, response.StatusCode);

            PagamentoDto? body = await response.Content.ReadFromJsonAsync<PagamentoDto>();
            body = Assert.IsType<PagamentoDto>(body);
            Assert.Equal(StatusPagamento.Pago, body.Status);
            Assert.Null(body.MovimentacaoId);
            Assert.Equal("Lancamento administrativo", body.Descricao);

            using IServiceScope scope = factory.Services.CreateScope();
            RenovaDbContext context = scope.ServiceProvider.GetRequiredService<RenovaDbContext>();
            Assert.Single(context.Pagamentos);
            Assert.Equal(45m, context.ClientesCreditos.Single(item => item.ClienteId == cliente.Id).Valor);
        }
